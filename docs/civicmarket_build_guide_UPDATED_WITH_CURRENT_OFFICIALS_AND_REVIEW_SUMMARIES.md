...
# ADDENDUM — Current Elected Officials + AI Review Summaries

See `CIVICMARKET_ADDENDUM_CURRENT_OFFICIALS_AND_REVIEW_SUMMARIES.md` for the full implementation plan, SQL migration, Claude Code prompts, testing checklist, and risk check.

This addendum is now part of the initial beta build and should be completed before continuing Week 4 through Week 8 UI work.

New hard blockers before beta invitations:
...
- Add `current_officials` table
- Add `review_summaries` table
- Add `officials_for_user` view
- Add `CurrentOfficialsSection` to Home and Profile
- Add `summarize-reviews` Edge Function
- Add AI Review Summary card above candidate and measure reviews
- Keep review summaries clearly labeled as community-opinion summaries, not official fact sources
